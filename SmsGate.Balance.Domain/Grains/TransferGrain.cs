...
public class TransferGrain : Grain, ITransferGrain
{
    ...
    {
        _logger.LogInformation($"{transCode} Transfer from: {fromAccount} to {toAccount} with amount: {amountToTransfer}");
        var result1 = 0m;
        try
        {
            result1 = await GrainFactory.GetGrain<IBalanceGrain>(fromAccount)
                .Withdraw(amountToTransfer, transCode);
            _logger.LogInformation($"{transCode} Transfer from: {fromAccount} to {toAccount} with amount: {amountToTransfer} success");
        }
        catch (Exception e)
        {
            _logger.LogError($"{transCode}-TransferGrain withdraw error: {e.Message}");
            return (-1, -1);
        }
        ...
        try
        {
            ...
            _logger.LogInformation($"{transCode} Deposit from: {fromAccount} to {toAccount} with amount: {amountToTransfer} success");
            return (result1, result2);
        }
        catch (Exception e)
        {
            _logger.LogError($"{transCode}-TransferGrain deposit error: {e.Message}");
            try
            {
                //refund
            ...
            {
                _logger.LogError($"{transCode}-TransferGrain refund error: {ex.Message}");
            }
