...
namespace SmsGate.Balance.Domain.Grains;
    ...
    : Grain<(IdempotencyShield shield, AccountBalanceDto account)>, IBalanceGrain
{
    ...
    async Task<SettlementDto> IBalanceGrain.Withdraw(SettlementDto settlement)
    {
        ...
        if (string.IsNullOrEmpty(settlement
                .DesAccountCode)) // Nếu DesAccount = null thì thì đánh dấu trạng thái settment về Done luôn
    ...
    async Task<decimal> IBalanceGrain.GetBalance()
    {
        ...
        {
            var grain = GrainFactory.GetGrain<IBalanceGrain>(State.account.AccountCode + "*" + i + "|" +
                                                             State.account.CurrencyCode);
    ...
    async Task<string> IBalanceGrain.GetShardAccount()
    {
        ...
        var shardId = ++State.account.CurrentShardCounter;
            ...
            finally
            {
                ...
                if (result) //update db thành công thì update state
                {
                    ...
                    if (!updateStateResult)
                    {
                        ...
                        {
                            logger.LogError("BlockBalance {TransCode} - {AccountCode} revert update data error",
                                State.account.AccountCode,
                                transCode);
                            //Hủy State hiện tại
                            DeactivateOnIdle();
            ...
            finally
            {
                ...
                if (result) //update db thành công thì update state
                {
                    ...
                    if (!updateStateResult)
                    {
                        ...
                        {
                            logger.LogError("UnBlockBalance {TransCode} - {AccountCode} revert update data error",
                                State.account.AccountCode,
                                transCode);
                            //Hủy State hiện tại
                            DeactivateOnIdle();
